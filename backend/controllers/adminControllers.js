import { ElectionDay } from "../models/adminModel.js";
...
export async function addElectionDate(req, res) {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Election date added successfully", newElection });
  } catch (error) {
    return res
      .status(500)
      .json({ message: "Unable to add election date", error: error.message });
  }
}
...
export async function applyCandidate(req, res) {
  ...
  try {
    ...
    if (alreadyExists) {
      return res
        .status(400)
        .json({ message: "You have already applied as a candidate" });
    }
    ...
    return res
      .status(500)
      .json({ message: "Unable to apply candidate", error: error.message });
  }
}
...
export async function addVotes(req, res) {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Unable to add vote", error: error.message });
  }
}
...
export async function setWinner(req, res) {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Winner set", election: updatedElection });
  } catch (error) {
